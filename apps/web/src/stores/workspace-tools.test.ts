...
import { WORKSPACE_TOOLS_PENDING_STORAGE_KEY, useWorkspaceToolsStore } from "./workspace-tools.js";
...
describe("Workspace tools transfer and permission store", () => {
  beforeEach(() => {
  ...
  it("persists only bounded transfer identifiers and resumes pending polling after reload", async () => {
    ...
    const first = useWorkspaceToolsStore();
    ...
    const serialized = localStorage.getItem(WORKSPACE_TOOLS_PENDING_STORAGE_KEY);
    ...
    const resumed = useWorkspaceToolsStore();
    ...
    expect(localStorage.getItem(WORKSPACE_TOOLS_PENDING_STORAGE_KEY)).toBeNull();
  });
  ...
  it("bounds repeated status polling and exposes an explicit paused state", async () => {
    ...
    localStorage.setItem(
      WORKSPACE_TOOLS_PENDING_STORAGE_KEY,
      JSON.stringify({
        version: 1,
  ...
  it("renders one sanitized permission state without provider or document detail", async () => {
    ...
    client.search.mockRejectedValue(
      Object.assign(
        new WorkspaceToolsApiError("NOTE_NOT_FOUND", 404, "77777777-7777-4777-8777-777777777777"),
    ...
    ).rejects.toBeInstanceOf(WorkspaceToolsApiError);
