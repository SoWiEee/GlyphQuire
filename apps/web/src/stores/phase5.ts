...
} from "../api/Phase5Client.js";

export const PHASE5_PENDING_STORAGE_KEY = "glyphquire.phase5.pending.v1";
...
export type Phase5PermissionState = "unknown" | "allowed" | "denied";
export type Phase5OperatorState = "unknown" | "member" | "operator";

export interface Phase5ClientPort {
...
export interface Phase5StoreOptions {
...
function publicError(error: unknown): { message: string; denied: boolean } {
  if (error instanceof Phase5ApiError) {
  ...
  if (
    error instanceof Phase5OfflineError ||
    error instanceof Phase5ValidationError ||
...
function defaultStorage(): Phase5StoreOptions["storage"] {
  return typeof localStorage === "undefined" ? undefined : localStorage;
}

export const usePhase5Store = defineStore("phase5", () => {
  const client = shallowRef<Phase5ClientPort>(new Phase5Client());
  ...
  const permissionState = ref<Phase5PermissionState>("unknown");
  const operatorState = ref<Phase5OperatorState>("unknown");
  ...
  function configure(nextClient: Phase5ClientPort, options: Phase5StoreOptions = {}): void {
    ...
    if (!Number.isInteger(nextInterval) || nextInterval < 10 || nextInterval > 60_000) {
      throw new Error("Invalid Phase 5 polling interval");
    }
    if (!Number.isInteger(nextMaximum) || nextMaximum < 1 || nextMaximum > 120) {
      throw new Error("Invalid Phase 5 polling bound");
    }
  ...
  function setOperatorState(next: Phase5OperatorState): void {
  ...
  function persistPending(): void {
    ...
    if (pending.value.length === 0) {
      storage.removeItem(PHASE5_PENDING_STORAGE_KEY);
      return;
    }
    const bounded = persistedPendingSchema.parse({ version: 1, transfers: pending.value });
    storage.setItem(PHASE5_PENDING_STORAGE_KEY, JSON.stringify(bounded));
  }
    ...
    try {
      if (attempted.kind === "import") {
        ...
        if (result.workspaceId !== attempted.workspaceId)
          throw new Phase5ValidationError("response");
  ...
  function loadPending(): PendingTransfer[] {
    if (!storage) return [];
    const serialized = storage.getItem(PHASE5_PENDING_STORAGE_KEY);
