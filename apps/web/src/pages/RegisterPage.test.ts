...
describe("RegisterPage", () => {
  ...
  it("shows the store error on failure", async () => {
    ...
    const wrapper = mount(RegisterPage, { attachTo: document.body });
    await wrapper.find("#name").setValue("Ada");
    await wrapper.find("#email").setValue("a@b.co");
    await wrapper.find("#password").setValue("pw");
    await wrapper.find("form").trigger("submit.prevent");
    ...
    expect(errorEl.text()).toContain("Email already registered");
    expect(errorEl.attributes("id")).toBe("register-error");
    ...
    expect(nameInput.attributes("aria-describedby")).toBe("register-error");
    ...
    expect(passwordInput.attributes("aria-describedby")).toBe("register-error");
