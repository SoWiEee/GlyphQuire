<template>
  <section aria-labelledby="custom-block-picker-heading" class="space-y-2">
    ...
    <p v-if="published.length === 0" class="text-xs text-muted">
    ...
    <ul v-else class="grid gap-1">
      <li v-for="record in published" :key="record.id">
        <button
          type="button"
          class="flex w-full items-center gap-2 rounded border px-2 py-2 text-left text-sm hover:bg-surface-muted"
        ...
        >
          ...
          <span class="text-[10px] uppercase text-muted">v{{ record.version }}</span>
        </button>
      </li>
    </ul>
  </section>
