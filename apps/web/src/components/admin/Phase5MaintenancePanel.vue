          ...
          >
            <span>
              ...
              <span class="block text-xs text-gray-600">
                Attempts: {{ item.attempts }}/{{ item.maxAttempts }} · Error: {{ item.errorCode }}
              </span>
            </span>
            <button
              type="button"
      ...
      >
        ...
        <ul v-else class="space-y-2 text-sm">
          <li v-for="item in backupVerifications" :key="item.jobId" class="rounded bg-gray-50 p-2">
            <span class="block">Status: {{ item.status }}</span>
            <span v-if="item.errorCode" class="block text-xs text-gray-600">
...
const props = withDefaults(
  defineProps<{
    workspaceId: string;
    client?: Phase5MaintenanceClient;
...
const api = props.client ?? new Phase5Client();
...
function handleFailure(cause: unknown): void {
  if (cause instanceof Phase5ApiError && cause.code === "NOTE_NOT_FOUND") {
    capabilities.value = [];
    reportDenied();
    return;
  }
