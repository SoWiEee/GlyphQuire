...
import { WorkspaceToolsApiError, type MaintenanceClient } from "../../api/WorkspaceToolsClient.js";
...
function client(overrides: Partial<MaintenanceClient> = {}): MaintenanceClient {
  return {
...
describe("MaintenancePanel", () => {
  ...
  it("renders no maintenance controls for a normal member and scrubs the denial", async () => {
    ...
    const api = client({
      getMaintenanceCapabilities: vi.fn(async () => {
        const error = Object.assign(new WorkspaceToolsApiError("NOTE_NOT_FOUND", 404, REQUEST_ID), {
          detail,
        });
        throw error;
      }),
  ...
  it("renders scrubbed diagnostics, replays by opaque id, and keeps pagination cursor bounded", async () => {
    ...
    expect(wrapper.text()).toContain("A maintenance task needs attention.");
    expect(wrapper.text()).toContain("Backup verification is in progress.");
    expect(wrapper.text()).not.toContain("JOB_FAILED");
