...
readonly EVIDENCE_FILE="${RESTORE_EVIDENCE_FILE:-${REPOSITORY_ROOT}/docs/evidence/phase5/backup-restore-drill.md}"
readonly PHASE6_OUTPUT_FILE="${PHASE6_BACKUP_EVIDENCE_FILE:-${REPOSITORY_ROOT}/docs/evidence/phase6/backup-restore.json}"
...
on_error() {
  ...
  if ((failure_reported == 0)); then
    failure_reported=1
    write_phase6_failure_evidence "${failure_reason:-INTERNAL_FAILURE}" >/dev/null 2>&1 || true
...
validate_isolated_database_target() {
  [[ -n "$RESTORE_DATABASE_URL" ]] || fail "RESTORE_DATABASE_URL_MISSING"
...
validate_isolated_object_target() {
  [[ -n "$RESTORE_OBJECT_STORAGE_TARGET" ]] || fail "RESTORE_OBJECT_STORAGE_TARGET_MISSING"
  if [[ "$RESTORE_OBJECT_STORAGE_TARGET" == s3://* ]]; then
    ...
    [[ -n "${AWS_ACCESS_KEY_ID:-${S3_ACCESS_KEY:-}}" ]] || fail "RESTORE_OBJECT_ACCESS_KEY_MISSING"
    [[ -n "${AWS_SECRET_ACCESS_KEY:-${S3_SECRET_KEY:-}}" ]] || fail "RESTORE_OBJECT_SECRET_KEY_MISSING"
    return
  fi
...
validate_manifest() {
  PHASE6_MANIFEST_FILE="$manifest_file" PHASE6_BACKUP_ID="$BACKUP_ID" PHASE6_KEY_VERSION="$BACKUP_ENCRYPTION_KEY_VERSION" \
    PHASE6_MAX_BYTES="$MAX_RESTORE_BYTES" PHASE6_MAX_FILES="$MAX_RESTORE_FILES" \
    node --input-type=module - <<'NODE'
import { createHash } from "node:crypto";
import { readFileSync } from "node:fs";

const env = process.env;
const manifest = JSON.parse(readFileSync(env.PHASE6_MANIFEST_FILE, "utf8"));
const sha256 = (value) => createHash("sha256").update(value).digest("hex");
const hashPattern = /^[a-f0-9]{64}$/;
const expectedVersions = ["0000", "0001", "0002", "0003", "0004", "0005", "0006", "0007", "0008", "0009", "0010", "0011"];
if (manifest.schemaVersion !== 1 || manifest.producer !== "phase6-backup-restore" || manifest.backupId !== env.PHASE6_BACKUP_ID) process.exit(1);
if (manifest.encryption?.algorithm !== "AES-256-GCM" || manifest.encryption?.authenticated !== true || manifest.encryption?.keyVersion !== env.PHASE6_KEY_VERSION) process.exit(1);
...
if (!manifest.bounds || manifest.bounds.maxBytes < 1 || manifest.bounds.maxFiles < 1 || manifest.bounds.maxBytes > Number(env.PHASE6_MAX_BYTES) || manifest.bounds.maxFiles > Number(env.PHASE6_MAX_FILES)) process.exit(1);
...
if (!Number.isInteger(objects.fileCount) || objects.fileCount < 0 || !Number.isInteger(objects.totalBytes) || objects.totalBytes < 0 || objects.fileCount > Number(env.PHASE6_MAX_FILES) || objects.totalBytes > Number(env.PHASE6_MAX_BYTES) || !hashPattern.test(objects.aggregateSha256 ?? "")) process.exit(1);
for (const artifact of [database.artifact, objects.artifact]) {
  if (!artifact || artifact.schemaVersion !== 1 || artifact.algorithm !== "AES-256-GCM" || artifact.authenticated !== true || artifact.keyVersion !== env.PHASE6_KEY_VERSION || artifact.kdf !== "scrypt" || !/^[a-f0-9]{32}$/.test(artifact.salt ?? "") || !/^[a-f0-9]{24}$/.test(artifact.iv ?? "") || !/^[a-f0-9]{32}$/.test(artifact.authTag ?? "") || !hashPattern.test(artifact.plaintextSha256 ?? "") || !hashPattern.test(artifact.ciphertextSha256 ?? "") || !Number.isInteger(artifact.plaintextBytes) || !Number.isInteger(artifact.ciphertextBytes)) process.exit(1);
}
...
write_phase6_evidence_record() {
  local status="$1"
  local blocking_reason="${2:-}"
  [[ "${PHASE6_BACKUP_EVIDENCE:-0}" == "1" || -n "${PHASE6_BACKUP_EVIDENCE_FILE:-}" ]] || return 0
  PHASE6_EVIDENCE_FILE="$PHASE6_OUTPUT_FILE" \
    PHASE6_STATUS="$status" \
    PHASE6_BLOCKING_REASON="$blocking_reason" \
    PHASE6_KEY_VERSION="$BACKUP_ENCRYPTION_KEY_VERSION" \
    PHASE6_MANIFEST_FILE="${manifest_file:-}" \
    PHASE6_MIGRATION_JOURNAL_SHA256="${migration_journal_sha256:-$ZERO_HASH}" \
    PHASE6_NOTES_ROWS="${notes_row_count:-0}" \
    PHASE6_NOTE_VERSIONS_ROWS="${note_versions_row_count:-0}" \
    PHASE6_ASSETS_ROWS="${assets_row_count:-0}" \
    PHASE6_OBJECT_COUNT="${object_count:-0}" \
    PHASE6_OBJECT_BYTES="${object_bytes:-0}" \
    PHASE6_OBJECT_AGGREGATE="${object_aggregate_sha256:-$ZERO_HASH}" \
    PHASE6_AGGREGATE="${aggregate_sha256:-$ZERO_HASH}" \
    node --input-type=module - <<'NODE'
import { chmodSync, existsSync, mkdirSync, readFileSync, renameSync, writeFileSync } from "node:fs";
import { dirname } from "node:path";

const env = process.env;
const outputPath = env.PHASE6_EVIDENCE_FILE;
const status = env.PHASE6_STATUS;
...
let manifest = null;
if (status === "passed") {
  if (!env.PHASE6_MANIFEST_FILE || !existsSync(env.PHASE6_MANIFEST_FILE)) throw new Error("passed evidence requires a manifest");
  manifest = JSON.parse(readFileSync(env.PHASE6_MANIFEST_FILE, "utf8"));
}

const history = [];
function historyEntry(record) {
  if (!record || record.schemaVersion !== 1 || !["blocked", "failed", "passed"].includes(record.status) || record.scrubbed !== true || record.target !== "isolated" || record.producer !== "phase6-backup-restore" || typeof record.capturedAt !== "string" || typeof record.recordedAt !== "string" || typeof record.aggregateSha256 !== "string" || !hashPattern.test(record.aggregateSha256)) throw new Error("existing evidence history is invalid");
  return {
    schemaVersion: 1,
...
const evidence = {
  ...
  target: "isolated",
  producer: "phase6-backup-restore",
  ...(env.PHASE6_BLOCKING_REASON ? { blockingReason: env.PHASE6_BLOCKING_REASON } : {}),
  capturedAt: now,
  encryption: { algorithm: "AES-256-GCM", authenticated: true, keyVersion: status === "passed" ? manifest.encryption.keyVersion : env.PHASE6_KEY_VERSION },
  retentionDays: 30,
  artifacts: {
    ...
    objectStorage: { authenticated: true, plaintextBytes: integerOrZero(objectArtifact.plaintextBytes), ciphertextBytes: integerOrZero(objectArtifact.ciphertextBytes), plaintextSha256: hashOrZero(objectArtifact.plaintextSha256), ciphertextSha256: hashOrZero(objectArtifact.ciphertextSha256), fileCount: status === "passed" ? integerOrZero(Number(env.PHASE6_OBJECT_COUNT)) : 0, totalBytes: status === "passed" ? integerOrZero(Number(env.PHASE6_OBJECT_BYTES)) : 0, aggregateSha256: status === "passed" ? hashOrZero(env.PHASE6_OBJECT_AGGREGATE) : zeroHash },
  },
  aggregateSha256: status === "passed" ? hashOrZero(env.PHASE6_AGGREGATE) : zeroHash,
  invariants: {
    migrationJournalSha256: status === "passed" ? hashOrZero(env.PHASE6_MIGRATION_JOURNAL_SHA256) : zeroHash,
    ...
    notesRows: status === "passed" ? integerOrZero(Number(env.PHASE6_NOTES_ROWS)) : 0,
    noteVersionsRows: status === "passed" ? integerOrZero(Number(env.PHASE6_NOTE_VERSIONS_ROWS)) : 0,
    assetsRows: status === "passed" ? integerOrZero(Number(env.PHASE6_ASSETS_ROWS)) : 0,
...
write_phase6_evidence() {
  write_phase6_evidence_record "$1"
}

write_phase6_failure_evidence() {
  write_phase6_evidence_record failed "$1"
}
