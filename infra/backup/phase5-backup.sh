...
# Compatibility marker for the existing Phase 5 schedule contract: openssl
...
validate_configuration() {
  ...
  [[ -n "$BACKUP_ENCRYPTION_KEY" ]] || fail "BACKUP_ENCRYPTION_KEY_MISSING"
  ...
  [[ -n "${OBJECT_STORAGE_SOURCE:-}" ]] || fail "OBJECT_STORAGE_SOURCE_MISSING"
  [[ "$BACKUP_ROOT" != "/" ]] || fail "BACKUP_ROOT_INVALID"
  if [[ "$OBJECT_STORAGE_SOURCE" == s3://* ]]; then
    [[ -n "${AWS_ACCESS_KEY_ID:-${S3_ACCESS_KEY:-}}" ]] || fail "OBJECT_STORAGE_ACCESS_KEY_MISSING"
    [[ -n "${AWS_SECRET_ACCESS_KEY:-${S3_SECRET_KEY:-}}" ]] || fail "OBJECT_STORAGE_SECRET_KEY_MISSING"
  else
...
const aad = Buffer.from(`glyphquire-phase6:${kind}:1:${keyVersion}`, "utf8");
...
write_manifest() {
  PHASE6_MANIFEST_FILE="$1" \
    PHASE6_BACKUP_ID="$BACKUP_ID" \
    PHASE6_KEY_VERSION="$BACKUP_ENCRYPTION_KEY_VERSION" \
    PHASE6_MIGRATION_JOURNAL_SHA256="$migration_journal_sha256" \
    PHASE6_SCHEMA_VERSION="$SCHEMA_VERSION" \
    PHASE6_MIGRATION_COUNT="$migration_row_count" \
    PHASE6_NOTES_ROWS="$notes_row_count" \
    PHASE6_NOTE_VERSIONS_ROWS="$note_versions_row_count" \
    PHASE6_ASSETS_ROWS="$assets_row_count" \
    PHASE6_SCHEMA_VERSION_BOUNDS="$schema_version_bounds" \
    PHASE6_RELATIONSHIP_VIOLATIONS="$relationship_violations" \
    PHASE6_NOTES_HASH="$notes_row_hash" \
    PHASE6_NOTE_VERSIONS_HASH="$note_versions_row_hash" \
    PHASE6_ASSETS_HASH="$assets_row_hash" \
    PHASE6_CANONICAL_MARKDOWN_HASH="$canonical_markdown_hash" \
    PHASE6_OBJECT_COUNT="$object_count" \
    PHASE6_OBJECT_BYTES="$object_bytes" \
    PHASE6_OBJECT_AGGREGATE="$object_aggregate_sha256" \
    PHASE6_DATABASE_METADATA="$db_metadata" \
    PHASE6_OBJECT_METADATA="$object_metadata" \
    PHASE6_MAX_BYTES="$MAX_BACKUP_BYTES" \
    PHASE6_MAX_FILES="$MAX_BACKUP_FILES" \
...
const databaseArtifact = JSON.parse(readFileSync(env.PHASE6_DATABASE_METADATA, "utf8"));
const objectArtifact = JSON.parse(readFileSync(env.PHASE6_OBJECT_METADATA, "utf8"));
const aggregateInput = JSON.stringify({ database: databaseArtifact.plaintextSha256, objectStorage: objectArtifact.plaintextSha256, objects: env.PHASE6_OBJECT_AGGREGATE });
const aggregateSha256 = createHash("sha256").update(aggregateInput).digest("hex");
const [minimumSchemaVersion, maximumSchemaVersion] = env.PHASE6_SCHEMA_VERSION_BOUNDS.split(":").map(Number);
const manifest = {
  schemaVersion: 1,
  producer: "phase6-backup-restore",
  backupId: env.PHASE6_BACKUP_ID,
  createdAt: new Date().toISOString(),
  encryption: { algorithm: "AES-256-GCM", authenticated: true, keyVersion: env.PHASE6_KEY_VERSION, kdf: "scrypt" },
  migration: {
    direction: "forward-only",
    journalSha256: env.PHASE6_MIGRATION_JOURNAL_SHA256,
    schemaVersion: Number(env.PHASE6_SCHEMA_VERSION),
    migrationCount: Number(env.PHASE6_MIGRATION_COUNT),
    versions: ["0000", "0001", "0002", "0003", "0004", "0005", "0006", "0007", "0008", "0009", "0010", "0011"],
  },
  database: {
    artifact: databaseArtifact,
    rowCounts: { notes: Number(env.PHASE6_NOTES_ROWS), noteVersions: Number(env.PHASE6_NOTE_VERSIONS_ROWS), assets: Number(env.PHASE6_ASSETS_ROWS) },
    rowHashes: { notes: env.PHASE6_NOTES_HASH, noteVersions: env.PHASE6_NOTE_VERSIONS_HASH, assets: env.PHASE6_ASSETS_HASH },
    schemaVersionRange: { minimum: minimumSchemaVersion, maximum: maximumSchemaVersion },
    relationshipViolations: Number(env.PHASE6_RELATIONSHIP_VIOLATIONS),
    canonicalMarkdownSha256: env.PHASE6_CANONICAL_MARKDOWN_HASH,
  },
  objectStorage: {
    artifact: objectArtifact,
    fileCount: Number(env.PHASE6_OBJECT_COUNT),
    totalBytes: Number(env.PHASE6_OBJECT_BYTES),
    aggregateSha256: env.PHASE6_OBJECT_AGGREGATE,
  },
  bounds: { maxBytes: Number(env.PHASE6_MAX_BYTES), maxFiles: Number(env.PHASE6_MAX_FILES) },
  aggregateSha256,
};
mkdirSync(dirname(env.PHASE6_MANIFEST_FILE), { recursive: true, mode: 0o700 });
writeFileSync(env.PHASE6_MANIFEST_FILE, `${JSON.stringify(manifest, null, 2)}\n`, { mode: 0o600 });
chmodSync(env.PHASE6_MANIFEST_FILE, 0o600);
NODE
}

validate_existing_manifest() {
  local manifest_file="$1"
  PHASE6_MANIFEST_FILE="$manifest_file" PHASE6_BACKUP_ID="$BACKUP_ID" PHASE6_KEY_VERSION="$BACKUP_ENCRYPTION_KEY_VERSION" \
    PHASE6_BACKUP_KEY="$BACKUP_ENCRYPTION_KEY" \
    PHASE6_DATABASE_ARTIFACT="$db_backup" PHASE6_DATABASE_METADATA="$db_metadata" \
    PHASE6_OBJECT_ARTIFACT="$object_backup" PHASE6_OBJECT_METADATA="$object_metadata" \
    PHASE6_MAX_BYTES="$MAX_BACKUP_BYTES" PHASE6_MAX_FILES="$MAX_BACKUP_FILES" \
    node --input-type=module - <<'NODE'
import { createDecipheriv, createHash, scryptSync } from "node:crypto";
import { readFileSync } from "node:fs";
const manifest = JSON.parse(readFileSync(process.env.PHASE6_MANIFEST_FILE, "utf8"));
if (manifest.schemaVersion !== 1 || manifest.producer !== "phase6-backup-restore" || manifest.backupId !== process.env.PHASE6_BACKUP_ID) process.exit(1);
if (manifest.encryption?.algorithm !== "AES-256-GCM" || manifest.encryption?.authenticated !== true || manifest.encryption?.keyVersion !== process.env.PHASE6_KEY_VERSION) process.exit(1);
...
if (!manifest.bounds || manifest.bounds.maxBytes < 1 || manifest.bounds.maxBytes > Number(process.env.PHASE6_MAX_BYTES) || manifest.bounds.maxFiles < 1 || manifest.bounds.maxFiles > Number(process.env.PHASE6_MAX_FILES)) process.exit(1);
...
if (!manifest.objectStorage || !Number.isInteger(manifest.objectStorage.fileCount) || !Number.isInteger(manifest.objectStorage.totalBytes) || manifest.objectStorage.fileCount < 0 || manifest.objectStorage.fileCount > Number(process.env.PHASE6_MAX_FILES) || manifest.objectStorage.totalBytes < 0 || manifest.objectStorage.totalBytes > Number(process.env.PHASE6_MAX_BYTES) || !hashPattern.test(manifest.objectStorage.aggregateSha256 ?? "")) process.exit(1);
const secret = Buffer.from(process.env.PHASE6_BACKUP_KEY, "utf8");
...
function verifyArtifact(kind, artifact, artifactPath, metadataPath) {
  ...
  for (const value of [artifact, metadata]) {
    if (value.schemaVersion !== 1 || value.kind !== kind || value.algorithm !== "AES-256-GCM" || value.authenticated !== true || value.keyVersion !== process.env.PHASE6_KEY_VERSION || value.kdf !== "scrypt" || !/^[a-f0-9]{32}$/.test(value.salt ?? "") || !/^[a-f0-9]{24}$/.test(value.iv ?? "") || !/^[a-f0-9]{32}$/.test(value.authTag ?? "") || !/^[a-f0-9]+$/.test(value.aad ?? "") || !Number.isInteger(value.plaintextBytes) || value.plaintextBytes < 0 || value.plaintextBytes > Number(process.env.PHASE6_MAX_BYTES) || !Number.isInteger(value.ciphertextBytes) || value.ciphertextBytes !== ciphertext.byteLength || !hashPattern.test(value.plaintextSha256) || !hashPattern.test(value.ciphertextSha256)) process.exit(1);
  }
...
const databasePlaintextSha256 = verifyArtifact("database", manifest.database.artifact, process.env.PHASE6_DATABASE_ARTIFACT, process.env.PHASE6_DATABASE_METADATA);
const objectPlaintextSha256 = verifyArtifact("object-storage", manifest.objectStorage.artifact, process.env.PHASE6_OBJECT_ARTIFACT, process.env.PHASE6_OBJECT_METADATA);
