...
readonly MIGRATIONS_DIR="${RELEASE_MIGRATIONS_DIR:-$REPOSITORY_ROOT/packages/database/src/migrations}"
readonly EVIDENCE_FILE="${RELEASE_EVIDENCE_FILE:-$REPOSITORY_ROOT/docs/evidence/release/deployment-rehearsal.json}"
...
RELEASE_MIGRATION_DATABASE_URL="${RELEASE_MIGRATION_DATABASE_URL:-}"
RELEASE_RUNTIME_ROLE="${RELEASE_RUNTIME_ROLE:-${RELEASE_EXPECTED_RUNTIME_ROLE:-}}"
RELEASE_MIGRATION_ROLE="${RELEASE_MIGRATION_ROLE:-${RELEASE_EXPECTED_MIGRATION_ROLE:-}}"
...
RELEASE_S3_ENDPOINT="${RELEASE_S3_ENDPOINT:-${RELEASE_OBJECT_STORAGE_URL:-}}"
RELEASE_EXPECTED_BUCKET="${RELEASE_EXPECTED_BUCKET:-${RELEASE_S3_BUCKET:-}}"
RELEASE_CANDIDATE_API_IMAGE="${RELEASE_CANDIDATE_API_IMAGE:-${RELEASE_CANDIDATE_API_DIGEST:-}}"
RELEASE_CANDIDATE_WEB_IMAGE="${RELEASE_CANDIDATE_WEB_IMAGE:-${RELEASE_CANDIDATE_WEB_DIGEST:-}}"
RELEASE_CANDIDATE_WORKER_IMAGE="${RELEASE_CANDIDATE_WORKER_IMAGE:-${RELEASE_CANDIDATE_WORKER_DIGEST:-}}"
...
RELEASE_CANDIDATE_SOURCE_SHA="${RELEASE_CANDIDATE_SOURCE_SHA:-${GITHUB_SHA:-}}"
...
fail() {
  ...
  printf 'RELEASE_DEPLOY_FAILED:%s\n' "$code" >&2
  exit 2
}

skip_external() {
  printf 'SKIPPED_EXTERNAL: %s\n' "${1:-external release target is unavailable}" >&2
  exit 2
}
...
require_database_target() {
  ...
  require_value RELEASE_MIGRATION_DATABASE_URL
  require_value RELEASE_RUNTIME_ROLE
  require_value RELEASE_MIGRATION_ROLE
  ...
  parse_database_url "$RELEASE_MIGRATION_DATABASE_URL" migration_user migration_host migration_name || fail "MIGRATION_DATABASE_URL_INVALID"
  [[ "$runtime_host" == "$RELEASE_EXPECTED_DATABASE_HOST" ]] || fail "DATABASE_HOST_NOT_CANONICAL"
  [[ "$migration_host" == "$RELEASE_EXPECTED_DATABASE_HOST" ]] || fail "MIGRATION_HOST_NOT_CANONICAL"
  [[ "$runtime_name" == "$RELEASE_EXPECTED_DATABASE_NAME" ]] || fail "DATABASE_NAME_NOT_CANONICAL"
  [[ "$migration_name" == "$RELEASE_EXPECTED_DATABASE_NAME" ]] || fail "MIGRATION_NAME_NOT_CANONICAL"
  [[ "$runtime_user" == "$RELEASE_RUNTIME_ROLE" ]] || fail "RUNTIME_ROLE_URL_MISMATCH"
  [[ "$migration_user" == "$RELEASE_MIGRATION_ROLE" ]] || fail "MIGRATION_ROLE_URL_MISMATCH"
  [[ "$RELEASE_RUNTIME_ROLE" != "$RELEASE_MIGRATION_ROLE" ]] || fail "MIGRATION_RUNTIME_ROLES_NOT_SEPARATE"
  [[ "$RELEASE_DATABASE_URL" != "$RELEASE_MIGRATION_DATABASE_URL" ]] || fail "MIGRATION_RUNTIME_URLS_NOT_SEPARATE"
  [[ "$RELEASE_RUNTIME_ROLE" =~ ^[a-z_][a-z0-9_]{0,62}$ ]] || fail "RUNTIME_ROLE_INVALID"
  [[ "$RELEASE_MIGRATION_ROLE" =~ ^[a-z_][a-z0-9_]{0,62}$ ]] || fail "MIGRATION_ROLE_INVALID"
}

require_object_target() {
  require_value RELEASE_S3_ENDPOINT
  require_value RELEASE_EXPECTED_BUCKET
  [[ "$RELEASE_S3_ENDPOINT" =~ ^https?://[^/@?#]+(:[0-9]+)?$ ]] || fail "OBJECT_STORAGE_ENDPOINT_INVALID"
  [[ "$RELEASE_EXPECTED_BUCKET" =~ ^[a-z0-9](\.?[a-z0-9-]+)*$ ]] || fail "OBJECT_STORAGE_BUCKET_INVALID"
}

require_release_identity() {
  [[ -z "${RELEASE_PREVIOUS_RELEASE_MANIFEST_URL:-}" ]] || fail "PREVIOUS_RELEASE_URL_NOT_ALLOWED"
  require_value RELEASE_CANDIDATE_SOURCE_SHA
  require_value RELEASE_PREVIOUS_RELEASE_SOURCE_SHA
  require_value RELEASE_PREVIOUS_RELEASE_MANIFEST_SHA256
  valid_git_sha "$RELEASE_CANDIDATE_SOURCE_SHA" || fail "CANDIDATE_SOURCE_SHA_INVALID"
  ...
  for image_name in \
    RELEASE_CANDIDATE_API_IMAGE RELEASE_CANDIDATE_WEB_IMAGE RELEASE_CANDIDATE_WORKER_IMAGE \
    RELEASE_PREVIOUS_API_IMAGE RELEASE_PREVIOUS_WEB_IMAGE RELEASE_PREVIOUS_WORKER_IMAGE; do
    require_value "$image_name"
    image_value="${!image_name}"
    valid_image_digest "$image_value" || fail "${image_name}_MUST_USE_IMMUTABLE_DIGEST"
  done
  [[ "$RELEASE_CANDIDATE_SOURCE_SHA" != "$RELEASE_PREVIOUS_RELEASE_SOURCE_SHA" ]] || fail "CANDIDATE_AND_PREVIOUS_RELEASE_MATCH"
}
...
run_database_preflight() {
  ...
  quiet psql "$RELEASE_MIGRATION_DATABASE_URL" -v ON_ERROR_STOP=1 -f "$REPOSITORY_ROOT/infra/postgres/upgrade/002_role_preflight.sql"
  quiet env MIGRATION_DATABASE_URL="$RELEASE_MIGRATION_DATABASE_URL" pnpm --filter @glyphquire/database migrate
  quiet env MIGRATION_DATABASE_URL="$RELEASE_MIGRATION_DATABASE_URL" pnpm --filter @glyphquire/database db:verify-baseline
}

run_object_preflight() {
  command -v aws >/dev/null 2>&1 || skip_external "AWS_CLI_MISSING"
  quiet env AWS_ENDPOINT_URL="$RELEASE_S3_ENDPOINT" \
    aws s3api head-bucket --bucket "$RELEASE_EXPECTED_BUCKET"
}
...
start_services() {
  ...
  export RELEASE_API_IMAGE="$api_image"
  export RELEASE_WEB_IMAGE="$web_image"
  export RELEASE_WORKER_IMAGE="$worker_image"
  compose_started=1
...
probe_services() {
  local name
  for name in RELEASE_API_BASE_URL RELEASE_PROBE_TOKEN RELEASE_READ_PROBE_PATH RELEASE_WRITE_PROBE_PATH; do
  ...
  quiet curl --fail --silent --show-error --max-time 5 \
    ...
    "$RELEASE_API_BASE_URL$RELEASE_READ_PROBE_PATH"
  quiet curl --fail --silent --show-error --max-time 5 \
    -X POST -H "Authorization: Bearer $RELEASE_PROBE_TOKEN" \
    -H 'content-type: application/json' --data '{"probe":"release"}' \
    "$RELEASE_API_BASE_URL$RELEASE_WRITE_PROBE_PATH"
}
...
write_evidence() {
  ...
  RELEASE_EVIDENCE_FILE="$EVIDENCE_FILE" \
    RELEASE_MIGRATIONS_DIR="$MIGRATIONS_DIR" \
    RELEASE_STATUS="$status" \
    RELEASE_MODE="$mode" \
    RELEASE_EXTERNAL="$external" \
    RELEASE_FROZEN="$frozen" \
    RELEASE_CANDIDATE_BOOT="$candidate_boot" \
    RELEASE_PREVIOUS_BOOT="$previous_boot" \
    RELEASE_PROBES="$probes" \
    RELEASE_CANDIDATE_API_IMAGE="$RELEASE_CANDIDATE_API_IMAGE" \
    RELEASE_CANDIDATE_WEB_IMAGE="$RELEASE_CANDIDATE_WEB_IMAGE" \
    RELEASE_CANDIDATE_WORKER_IMAGE="$RELEASE_CANDIDATE_WORKER_IMAGE" \
    ...
    RELEASE_CANDIDATE_SOURCE_SHA="$RELEASE_CANDIDATE_SOURCE_SHA" \
    ...
    node --input-type=module -e '
      ...
      const evidence = {
        ...
        externalEvidenceAvailable: boolean(process.env.RELEASE_EXTERNAL),
        candidate: { sourceSha: process.env.RELEASE_CANDIDATE_SOURCE_SHA, api: digest(process.env.RELEASE_CANDIDATE_API_IMAGE), web: digest(process.env.RELEASE_CANDIDATE_WEB_IMAGE), worker: digest(process.env.RELEASE_CANDIDATE_WORKER_IMAGE) },
        ...
        migration: { journalSha256, snapshotSha256, artifacts, frozenByteIdentical: boolean(process.env.RELEASE_FROZEN) },
        checks: { preflight: boolean(process.env.RELEASE_EXTERNAL), migration: boolean(process.env.RELEASE_EXTERNAL), candidateBoot: boolean(process.env.RELEASE_CANDIDATE_BOOT), previousBoot: boolean(process.env.RELEASE_PREVIOUS_BOOT), compatibility: boolean(process.env.RELEASE_PROBES), noHistoryRewrite: boolean(process.env.RELEASE_FROZEN) },
        probes: { read: boolean(process.env.RELEASE_PROBES), write: boolean(process.env.RELEASE_PROBES) },
...
work_dir="$(mktemp -d "${TMPDIR:-/tmp}/glyphquire-release-deploy.XXXXXX")"
...
if [[ "$RELEASE_DRY_RUN" == "1" ]]; then
  ...
  printf '{"event":"RELEASE_DEPLOYMENT_REHEARSAL","status":"blocked","target":"isolated","scrubbed":true}\n'
  exit 0
fi
...
start_services "$RELEASE_CANDIDATE_API_IMAGE" "$RELEASE_CANDIDATE_WEB_IMAGE" "$RELEASE_CANDIDATE_WORKER_IMAGE"
probe_services
capture_frozen_hashes "$after_candidate_hashes"
assert_frozen_hashes "$before_hashes" "$after_candidate_hashes"
stop_services

start_services "$RELEASE_PREVIOUS_API_IMAGE" "$RELEASE_PREVIOUS_WEB_IMAGE" "$RELEASE_PREVIOUS_WORKER_IMAGE"
probe_services
...
printf '{"event":"RELEASE_DEPLOYMENT_REHEARSAL","status":"passed","target":"isolated","scrubbed":true}\n'
