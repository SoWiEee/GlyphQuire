...
try {
  const checklistPath = resolve(root, "docs/evidence/release/p0-release-checklist.md");
  if (!existsSync(checklistPath)) fail("P0 checklist is missing");
  else hasExactGateRows(await readFile(checklistPath, "utf8"));

  const manifest = await json("docs/evidence/release/artifact-manifest.json");
  if (manifest) {
  ...
  const browser = await json("docs/evidence/release/browser-matrix.json");
  ...
  const performance = await json("docs/evidence/release/performance-environment.json");
  const load = await json("docs/evidence/release/performance-load.json");
  ...
  const alert = await json("docs/evidence/release/alert-evidence.json");
  ...
  for (const [file, platform, reader] of screenReaders) {
    const value = await json(`docs/evidence/release/${file}`);
  ...
  const candidateMode = process.env.RELEASE_CANDIDATE === "1";
  if (candidateMode) {
    const { execFileSync } = await import("node:child_process");
    const status = execFileSync("git", ["status", "--porcelain"], { cwd: root, encoding: "utf8" }).trim();
    if (status) fail("release candidate checkout is not clean");
    const head = execFileSync("git", ["rev-parse", "HEAD"], { cwd: root, encoding: "utf8" }).trim();
    if (process.env.RELEASE_CANDIDATE_SOURCE_SHA !== head) fail("candidate source SHA does not match HEAD");
  }
} catch (error) {
...
const publicationSha = process.env.RELEASE_EVIDENCE_PUBLICATION_SHA;
const approval = process.env.RELEASE_APPROVAL;
if (!sha40.test(publicationSha ?? "") || !approval) {
  console.error("RELEASE_BLOCKED: evidence publication SHA and explicit approval are required");
  process.exit(2);
}
if (process.env.RELEASE_EMIT_DECISION !== "1") {
  console.log("RELEASE_VALIDATED_NO_DECISION");
  process.exit(0);
}
const decision = {
  rows: requiredGates.map((gate) => ({ gate, status: "passed" })),
  artifactManifest: JSON.parse(await readFile(resolve(root, "docs/evidence/release/artifact-manifest.json"), "utf8")),
  evidencePublicationSha: publicationSha,
};
const output = resolve(root, "docs/evidence/release/release-decision.json");
...
console.log("RELEASE_DECISION_WRITTEN");
NODE
