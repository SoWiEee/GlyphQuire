...
} from "./release-alert-runtime.js";

const MAX_BODY_BYTES = 16 * 1024;
const EVIDENCE_PRODUCER = "release-observability";
const canonicalUuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/u;

export interface ReleaseAlertDeliveredEvent extends ReleaseAlertEvent {
  deliveredAt: string;
  deliveryStatus: number;
}

export interface ReleaseAlertEvidence {
  schemaVersion: 1;
  ...
  events: ReleaseAlertDeliveredEvent[];
}

export interface ReleaseAlertReceiverOptions {
  capturePath?: string;
  clock?: () => number;
  deliveryStatus?: number;
}

export interface ReleaseAlertReceipt {
  accepted: boolean;
  statusCode: number;
  evidence?: ReleaseAlertEvidence;
}
...
function emptyEvidence(now: number): ReleaseAlertEvidence {
  return {
    schemaVersion: 1,
    status: "blocked",
...
function isDeliveredEvent(value: unknown): value is ReleaseAlertDeliveredEvent {
  ...
  return (
    ...
    (RELEASE_ALERT_NAMES as readonly string[]).includes(record.alert) &&
...
async function readEvidence(path: string, now: number): Promise<ReleaseAlertEvidence> {
  try {
...
async function writeEvidence(path: string, evidence: ReleaseAlertEvidence): Promise<void> {
...
function eventStatus(
  events: readonly ReleaseAlertDeliveredEvent[],
): ReleaseAlertEvidence["status"] {
...
export async function receiveReleaseAlertEvent(
  input: unknown,
  options: ReleaseAlertReceiverOptions = {},
): Promise<ReleaseAlertReceipt> {
  const clock = options.clock ?? Date.now;
  const capturePath = resolve(
    options.capturePath ??
      process.env.RELEASE_ALERT_EVIDENCE_HOST_PATH ??
      "/tmp/release-alert-evidence.json",
  );
  ...
  const event = sanitizeReleaseAlertInput(input, now);
  const detectedAt = Date.parse(event.detectedAt);
  if (
    !Number.isFinite(detectedAt) ||
    now < detectedAt ||
    now - detectedAt > RELEASE_ALERT_POLICY.deliveryDeadlineMs
  ) {
    return { accepted: false, statusCode: 422 };
  }
  const delivered: ReleaseAlertDeliveredEvent = {
    ...event,
 ...
 */
export function createReleaseAlertReceiverServer(
  options: ReleaseAlertReceiverOptions = {},
): Server {
  const clock = options.clock ?? Date.now;
  const capturePath = resolve(
  ...
  return createServer(async (request, response) => {
    ...
    if (request.method === "GET" && (path === "/ready" || path === "/health")) {
      writeJson(response, 200, { status: "ready", service: "release-alert-receiver" });
      return;
    }
    ...
    const receipt = await receiveReleaseAlertEvent(input, { capturePath, clock, deliveryStatus });
    if (!receipt.accepted) {
...
export function startReleaseAlertReceiverServer(options: ReleaseAlertReceiverOptions = {}): Server {
  const port = Number(process.env.RELEASE_ALERT_RECEIVER_PORT ?? 8080);
  const server = createReleaseAlertReceiverServer(options);
...
if (
  ...
  startReleaseAlertReceiverServer();
}
