    ...
    switch (node.type) {
      case "tabs":
        ...
        for (const child of node.children)
          walk([child], diagnostics, { parent: "tabs", suppressInvalidParent: false });
        break;
      case "columns":
        for (const child of node.children)
          walk([child], diagnostics, { parent: "columns", suppressInvalidParent: false });
        break;
      case "tab":
      ...
      case "invalid-block": {
        const recoveredParent =
          node.originalType === "tabs" || node.originalType === "columns"
            ? node.originalType
            : undefined;
        walk(node.children, diagnostics, {
