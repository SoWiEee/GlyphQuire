...
Run `infra/backup/pre-destructive-hook.sh` before migrations or purge
operations. Daily backups are encrypted with `BACKUP_ENCRYPTION_KEY` and
retained for 30 days by `backup.sh`; failures emit only the scrubbed
`BACKUP_FAILED` event and exit non-zero. Use `restore-drill.sh` with an
