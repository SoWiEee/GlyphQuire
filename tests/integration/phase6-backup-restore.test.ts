...
const backupScript = resolve(repositoryRoot, "infra/backup/phase5-backup.sh");
const restoreScript = resolve(repositoryRoot, "infra/backup/phase5-restore-drill.sh");
...
async function disposableEnvironment() {
  const root = await mkdtemp(resolve(tmpdir(), "glyphquire-phase6-backup-"));
  ...
  const phase5EvidencePath = resolve(root, "backup-restore.md");
  ...
  const base: NodeJS.ProcessEnv = {
    ...
    BACKUP_ENCRYPTION_KEY: "phase6-test-key-that-is-not-evidence",
    ...
    RESTORE_DATABASE_URL: "postgresql://restore:secret@db.example/phase6_restore",
    ...
    RESTORE_EVIDENCE_FILE: phase5EvidencePath,
    PHASE6_BACKUP_EVIDENCE: "1",
    PHASE6_BACKUP_EVIDENCE_FILE: evidencePath,
...
describe("Phase 6 encrypted backup and isolated restore contract", () => {
  it("uses an authenticated AES-256-GCM envelope with versioned key metadata", async () => {
    const script = await source("infra/backup/phase5-backup.sh");
  ...
  it("confines restore to explicitly confirmed isolated database and object targets", async () => {
    const script = await source("infra/backup/phase5-restore-drill.sh");
  ...
  it("checks forward-only migrations, schema, relationships, rows, hashes, and canonical Markdown", async () => {
    const [backup, restore] = await Promise.all([
      source("infra/backup/phase5-backup.sh"),
      source("infra/backup/phase5-restore-drill.sh"),
    ]);
  ...
  it("defines a strict sanitized evidence schema and instance", async () => {
    const [schemaSource, evidenceSource] = await Promise.all([
      source("docs/evidence/phase6/backup-restore-evidence.schema.json"),
      source("docs/evidence/phase6/backup-restore.json"),
    ]);
