...
import type {
  Phase6PreflightChecks,
  Phase6PreflightExpected,
  Phase6PreflightProbe,
} from "../../apps/api/src/routes/internal-phase6-preflight.js";
import { createPhase6PreflightRoutes } from "../../apps/api/src/routes/internal-phase6-preflight.js";

const baseUrl = "http://localhost:3000";
const actorId = "configured-phase6-operator";

const expected: Phase6PreflightExpected = {
  runtimeRole: "glyphquire_app",
  migrationRole: "glyphquire_migration",
  workerId: "worker-phase6-01",
  bucket: "glyphquire-private",
  imageDigest: `sha256:${"a".repeat(64)}`,
  migrationJournalSha: "b".repeat(64),
};

const passingChecks: Phase6PreflightChecks = {
  health: true,
  readiness: true,
  database: true,
  objectStorage: true,
...
function app(options: {
  ...
  const routes = createPhase6PreflightRoutes({
...
describe("internal Phase 6 preflight route", () => {
  it("returns only passing booleans and expected identities to an exact operator", async () => {
    ...
    const response = await instance.request(`${baseUrl}/api/internal/phase6/preflight`);
  ...
  it("accepts only the vault probe bearer and still applies operator authorization", async () => {
    const probeToken = "phase6-probe-token";
    const { app: instance, authorize } = app({
      probeToken,
      probeOperatorId: actorId,
    });

    const denied = await instance.request(`${baseUrl}/api/internal/phase6/preflight`, {
      headers: { authorization: "Bearer wrong-token" },
    });
    expect(denied.status).toBe(404);

    const accepted = await instance.request(`${baseUrl}/api/internal/phase6/preflight`, {
