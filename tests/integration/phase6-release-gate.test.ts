...
const decisionPath = resolve(root, "docs/evidence/phase6/release-decision.json");

describe("Phase 6 release decision gate", () => {
  it("requires all fourteen P0 rows and never emits a decision for blocked evidence", () => {
    const result = spawnSync("bash", [gatePath], {
      ...
      env: {
        ...process.env,
        PHASE6_RELEASE_CANDIDATE: "0",
        PHASE6_EMIT_DECISION: "1",
        PHASE6_EVIDENCE_PUBLICATION_SHA: "a".repeat(40),
        PHASE6_RELEASE_APPROVAL: "test-only",
      },
    });
    ...
    expect(result.status).toBe(2);
    expect(`${result.stdout}\n${result.stderr}`).toMatch(/P0-01|PHASE6_RELEASE_BLOCKED/u);
