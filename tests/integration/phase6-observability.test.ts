...
import {
  PHASE6_ALERT_POLICY,
  Phase6AlertEvaluator,
  createPhase6AlertEvaluator,
  schedulePhase6Probes,
  sanitizeAlertInput,
  type AlertEvent,
} from "../../infra/observability/phase6-alert-runtime.js";
import { receiveAlertEvent } from "../../infra/observability/phase6-alert-receiver.js";
...
function eventNames(events: readonly AlertEvent[]) {
  return events.map((event) => `${event.phase}:${event.alert}`);
}

describe("Phase 6 alert evaluator", () => {
  it("uses a 30-second probe cadence and five-second timeout", () => {
    expect(PHASE6_ALERT_POLICY).toMatchObject({
    ...
    expect(new Phase6AlertEvaluator().policy).toEqual(PHASE6_ALERT_POLICY);
  });

  it("runs the configured probe exactly every 30 seconds and times out at five seconds", async () => {
    vi.useFakeTimers();
    try {
      const evaluator = createPhase6AlertEvaluator({ clock: () => baseTime });
      let probeCalls = 0;
      const stop = schedulePhase6Probes(evaluator, () => {
        probeCalls += 1;
        return true;
      });
  ...
  it("sanitizes secrets, URLs, bodies, and provider responses before transport", () => {
    const event = sanitizeAlertInput({
      alert: "dead_letter",
      phase: "firing",
      severity: "critical",
      requestId,
...
describe("Phase 6 API operational seams", () => {
...
describe("Phase 6 evidence and worker alert wiring", () => {
  it("delivers firing and recovery events within five minutes without echoing payloads", async () => {
    const directory = await mkdtemp(join(tmpdir(), "glyphquire-phase6-alert-"));
    const capturePath = join(directory, "alert-evidence.json");
    let now = baseTime;
    try {
      const firing = await receiveAlertEvent(
        {
          phase: "firing",
      ...
      now += 3 * 60_000;
      const recovery = await receiveAlertEvent(
        {
          phase: "resolved",
          alert: "backup_failure",
          severity: "critical",
