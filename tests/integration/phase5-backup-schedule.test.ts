...
describe("Phase 5 backup schedule and destructive guard", () => {
  it("runs an encrypted backup daily with strict failure and 30-day retention", async () => {
    const [script, service, timer] = await Promise.all([
      ...
      source("infra/backup/phase5-backup.service"),
      source("infra/backup/phase5-backup.timer"),
    ]);
    ...
    expect(service).toContain("ExecStart=/opt/glyphquire/infra/backup/phase5-backup.sh");
    expect(timer).toContain("OnCalendar=daily");
    expect(timer).toContain("Persistent=true");
  });

  it("blocks a destructive operation when its preflight backup fails", async () => {
    const hook = await source("infra/backup/phase5-pre-destructive-hook.sh");
    expect(hook).toContain("set -Eeuo pipefail");
    expect(hook).toContain("phase5-backup.sh");
