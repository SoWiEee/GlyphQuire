 ...
 *   WORKSPACE_LOAD_BASE_URL=https://staging.example
 *   WORKSPACE_LOAD_ACTORS_JSON='[{"workspaceId":"...","noteId":"...","cookie":"..."}, ...]'
 *   WORKSPACE_LOAD_OPERATOR_COOKIE='session=...'
 */
...
import {
  ...
  type ReleaseEnvironmentImageDigests,
...
const DEFAULT_PERFORMANCE_EVIDENCE_PATH = "docs/evidence/release/performance-load.json";
...
export interface WorkspaceLoadConfig {
...
export interface WorkspaceLoadIdentity {
  ...
  readonly imageDigests: ReleaseEnvironmentImageDigests | null;
  readonly host: ReleaseEnvironmentManifest["host"] | null;
}
...
export interface WorkspaceLoadReport {
  ...
  readonly producer: "release-performance-load";
  readonly recordedAt: string;
  readonly blockingReason?: string;
  readonly identity: WorkspaceLoadIdentity;
  readonly workload: {
    readonly durationMs: number;
  ...
  readonly api: {
    readonly getNote: ReturnType<typeof summarizeReleaseSamples> & { readonly thresholdMs: 500 };
    readonly autosave: ReturnType<typeof summarizeReleaseSamples> & { readonly thresholdMs: 1_000 };
    readonly search: ReturnType<typeof summarizeReleaseSamples> & { readonly thresholdMs: 500 };
  };
  readonly integrity: {
...
export function parseWorkspaceDuration(value: string): number {
...
export function parseWorkspaceCliConfig(argumentsToParse: readonly string[]): WorkspaceLoadConfig {
  let durationMs = DEFAULT_DURATION_MS;
  let users = DEFAULT_USERS;
  let environmentManifestPath: string | undefined;
  for (const argument of argumentsToParse) {
    if (argument === "--") continue;
    if (argument.startsWith("--duration=")) durationMs = parseWorkspaceDuration(argument.slice(11));
...
function cliConfig(): WorkspaceLoadConfig {
  return parseWorkspaceCliConfig(process.argv.slice(2));
}
...
function assertStrictReleaseManifestShape(
  value: unknown,
): asserts value is ReleaseEnvironmentManifest {
  const required = [
    "schemaVersion",
    "status",
    "scrubbed",
...
function expectedImageDigests(): ReleaseEnvironmentImageDigests {
...
export async function loadReleaseEnvironmentManifest(
  manifestPath: string,
): Promise<ReleaseEnvironmentManifest> {
  ...
  assertStrictReleaseManifestShape(parsed);
  const measurement = await measureReleaseEnvironment();
  return validateReleaseEnvironmentManifest(parsed, {
...
function buildMarkdown(marker: string, tick: number): string {
  const prefix = `# workspace load ${marker}\n\ntick ${tick}\n\n`;
...
export function summarizeReleaseSamples(samples: readonly number[]) {
  return {
...
function emptyLoadIdentity(): WorkspaceLoadIdentity {
  return {
...
function manifestLoadIdentity(manifest: ReleaseEnvironmentManifest): WorkspaceLoadIdentity {
  return {
...
function buildPerformanceReport(
  config: WorkspaceLoadConfig,
  identity: WorkspaceLoadIdentity,
  metrics: Metrics,
  warmupMs: number,
  newDeadLetters: number,
  searchFreshWithin60Seconds: boolean,
  status: WorkspaceLoadReport["status"],
  ...
  return {
    ...
    status,
    scrubbed: true,
    producer: "release-performance-load",
    ...
    api: {
      getNote: { ...summarizeReleaseSamples(metrics.getNote), thresholdMs: 500 },
      autosave: { ...summarizeReleaseSamples(metrics.autosave), thresholdMs: 1_000 },
      search: { ...summarizeReleaseSamples(metrics.search), thresholdMs: 500 },
    },
    integrity: {
...
async function writePerformanceReport(
  ...
  path = process.env.RELEASE_PERFORMANCE_LOAD_EVIDENCE_FILE ?? DEFAULT_PERFORMANCE_EVIDENCE_PATH,
): Promise<void> {
...
export class WorkspaceLoadBlockedError extends Error {
  constructor(message: string) {
    super(message);
    this.name = "WorkspaceLoadBlockedError";
  }
}
...
async function run(): Promise<void> {
  ...
  const baseRaw = process.env.WORKSPACE_LOAD_BASE_URL;
  const actorsRaw = process.env.WORKSPACE_LOAD_ACTORS_JSON;
  const operatorCookie = process.env.WORKSPACE_LOAD_OPERATOR_COOKIE;
  if (!baseRaw || !actorsRaw || !operatorCookie) {
    console.error(
      "SKIPPED_EXTERNAL: WORKSPACE_LOAD_BASE_URL, WORKSPACE_LOAD_ACTORS_JSON, and WORKSPACE_LOAD_OPERATOR_COOKIE are required",
    );
    process.exitCode = 2;
    return;
  }
  if (productionProfile && !config.environmentManifestPath) {
    ...
    await writePerformanceReport(
      buildPerformanceReport(
        ...
        false,
        "blocked",
        "measured release environment manifest is required for the release profile",
      ),
    );
    console.error(
      "SKIPPED_EXTERNAL: --environment-manifest=<path> is required for the 30m/5-user release profile",
    );
    process.exitCode = 2;
    return;
  }
  ...
  let manifest: ReleaseEnvironmentManifest | undefined;
  if (config.environmentManifestPath) {
    try {
      manifest = await loadReleaseEnvironmentManifest(config.environmentManifestPath);
    } catch {
      if (productionProfile) {
        ...
        await writePerformanceReport(
          buildPerformanceReport(
            ...
            false,
            "blocked",
            "measured release environment manifest is unavailable or invalid",
          ),
        );
        console.error(
          "SKIPPED_EXTERNAL: measured release environment manifest is unavailable or invalid",
        );
        process.exitCode = 2;
        return;
      }
...
if (invokedScript === resolve(import.meta.filename)) {
  run().catch((error: unknown) => {
    if (error instanceof WorkspaceLoadBlockedError) {
      console.error(`SKIPPED_EXTERNAL: ${error.message}`);
      process.exitCode = 2;
      return;
    }
    console.error(
      "WORKSPACE_LOAD_FAILED: see sanitized counters; credentials and document content omitted",
    );
    process.exitCode = 1;
  });
}
