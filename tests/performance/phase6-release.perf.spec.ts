...
} from "../load/phase6-environment.js";

const REPOSITORY_ROOT = resolve(import.meta.dirname, "../..");
const DEFAULT_ENVIRONMENT_MANIFEST = "docs/evidence/phase6/performance-environment.json";
const DEFAULT_LOAD_EVIDENCE = "docs/evidence/phase6/performance-load.json";
const SOURCE_SHA_PATTERN = /^[a-f0-9]{40}$/u;
const IMAGE_DIGEST_PATTERN = /^sha256:[a-f0-9]{64}$/u;

export const PHASE6_PERFORMANCE_WORKLOAD = {
  actors: 5,
  workspaces: 5,
...
interface Phase6Actor {
  readonly workspaceId: string;
  readonly noteId: string;
  readonly cookie: string;
}

interface Phase6LoadEvidence {
...
export function summarizePhase6Samples(samples: readonly number[]): SampleSummary {
  return {
...
function manifestPath(): string {
  return process.env.PHASE6_ENVIRONMENT_MANIFEST ?? DEFAULT_ENVIRONMENT_MANIFEST;
}

function loadEvidencePath(): string {
  return (
    process.env.PHASE6_PERFORMANCE_LOAD_EVIDENCE ??
    process.env.PHASE6_PERFORMANCE_LOAD_EVIDENCE_FILE ??
...
async function loadManifestAt(path: string): Promise<Phase6EnvironmentManifest> {
  let value: unknown;
  try {
    value = JSON.parse(await readFile(resolve(REPOSITORY_ROOT, path), "utf8"));
  } catch {
    throw new Error("measured Phase 6 environment manifest is unavailable");
  }
  const measurement = await measurePhase6Environment();
  return validatePhase6EnvironmentManifest(value, {
...
export async function loadPhase6ReleaseManifest(): Promise<Phase6EnvironmentManifest> {
  return loadManifestAt(manifestPath());
}

function parseActors(): Phase6Actor[] {
  const raw = process.env.PHASE5_LOAD_ACTORS_JSON;
  if (!raw) throw new Error("five Phase 5 load actors are required");
  let value: unknown;
  try {
  ...
  if (!Array.isArray(value) || value.length !== PHASE6_PERFORMANCE_WORKLOAD.actors) {
...
async function verifySeededNotes(request: APIRequestContext, actor: Phase6Actor): Promise<void> {
  ...
  for (let page = 0; page < 10; page += 1) {
    ...
    if (count > PHASE6_PERFORMANCE_WORKLOAD.notesPerWorkspace) {
      throw new Error("workspace contains more than 1,000 notes");
    }
    if (!parsed.data.nextCursor) break;
    cursor = parsed.data.nextCursor;
  }
  expect(count).toBe(PHASE6_PERFORMANCE_WORKLOAD.notesPerWorkspace);
...
async function longTasksDuringTyping(page: Page): Promise<number[]> {
  await page.evaluate(() => {
    (window as unknown as { __phase6LongTasks?: number[] }).__phase6LongTasks = [];
    new PerformanceObserver((list) => {
      const values = (window as unknown as { __phase6LongTasks: number[] }).__phase6LongTasks;
  ...
  await page
    ...
    .pressSequentially("phase6 workload typing ".repeat(200));
  return page.evaluate(
    () => (window as unknown as { __phase6LongTasks: number[] }).__phase6LongTasks,
  );
}

test.describe("Phase 6 performance harness contract", () => {
  test("declares the exact five-actor, 1,000-note workload and UI gates", () => {
    expect(PHASE6_PERFORMANCE_WORKLOAD).toMatchObject({
      actors: 5,
      workspaces: 5,
      notesPerWorkspace: 1_000,
    });
    expect(
      PHASE6_UI_GATES.map((gate) => [gate.id, gate.warmups, gate.samples, gate.thresholdMs]),
    ).toEqual([
    ...
    expect(
      PHASE6_API_GATES.map((gate) => [gate.route, gate.minimumSamples, gate.thresholdMs]),
    ).toEqual([
      ["getNote", 500, 500],
      ["autosave", 500, 1_000],
      ["search", 500, 500],
  ...
  test("computes nearest-rank p50, p95, and p99 summaries", () => {
    expect(summarizePhase6Samples(Array.from({ length: 100 }, (_, index) => index + 1))).toEqual({
      count: 100,
      p50Ms: 50,
      p95Ms: 95,
      p99Ms: 99,
  ...
  test("blocks when the measured environment manifest is unavailable", async () => {
    await expect(
      loadManifestAt("docs/evidence/phase6/manifest-does-not-exist.json"),
...
test.describe("Phase 6 release performance gates", () => {
  test.skip(
    !environmentManifestAvailable,
    "PHASE6_PERFORMANCE_BLOCKED: measured environment manifest unavailable",
  );
  test.describe.configure({ mode: "serial" });

  let manifest: Phase6EnvironmentManifest;
  let actors: Phase6Actor[];

  test.beforeAll(async ({ request }) => {
    manifest = await loadPhase6ReleaseManifest();
    actors = parseActors();
    ...
    for (const gate of PHASE6_API_GATES) {
  ...
  test("PERF-UI-01: 100 KB input — InputEvent dispatch to next rendered animation frame, p95 < 100ms", async ({
    ...
    expect(summarizePhase6Samples(samples).p95Ms).toBeLessThan(100);
  });
  ...
  test("PERF-UI-02: Visual/Source switch — trigger to target editor accepts input, p95 < 1s", async ({
    ...
    expect(summarizePhase6Samples(samples).p95Ms).toBeLessThan(1_000);
  });
  ...
  test("PERF-UI-03: 1 MB open — request dispatch to editor accepting input, p95 < 5s", async ({
    ...
    if (!actor) throw new Error("phase 6 actor is unavailable");
    ...
    expect(samples).toHaveLength(100);
    expect(summarizePhase6Samples(samples).p95Ms).toBeLessThan(5_000);
  });
